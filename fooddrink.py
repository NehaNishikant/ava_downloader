...
def toOneHot(labels, k=2):
    N = labels.shape[0]
    oneHots = torch.zeros((N, k))
    for i in range(N):
        idx = labels[i].item()
        oneHots[i][int(idx)] = 1
    print(oneHots)
...
class Net(nn.Module):
    def __init__(self):
        ...
        #VGG GANG
        # 6 x 128 x 128
        self.conv1a = nn.Conv2d(6, 16, 3, stride=1, padding=1) # 16 x 128 x 128
        ...
        self.fc2 = nn.Linear(10, 2) #classifies between 2
...
trainX = torch.load("trainX2.pt")
trainY = torch.load("trainY2.pt")
trainset = TensorDataset(trainX, trainY)

testX = torch.load("testX2.pt")
testY = torch.load("testY2.pt")
...
for epoch in range(epochs):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader):
        ...
        loss = lossfn(outputs, toOneHot(labels))
        loss.backward()
        optimizer.step()
        
        # print statistics
        running_loss_train += loss.item()
        running_acc_train += (torch.argmax(outputs, dim=1) == labels[:, 0]).sum().item()
    ...
    for i, data in enumerate(testloader):
        ...
        inputs, labels = data
        outputs = net(inputs)
        loss = lossfn2(outputs, toOneHot(labels))
        
        running_loss_test += loss.item()
        running_acc_test += (torch.argmax(outputs, dim=1) == labels[:, 0]).sum().item()
